...
func TestReloadRemoteClusterSecret(t *testing.T) {
	...
	updatedKubeConfigRaw, err := clientcmd.Write(*kubeConfig)
	require.NoError(err)

	createTestRemoteClusterSecret(t, testClusterName, string(updatedKubeConfigRaw))
	...
	require.NoError(err)
	check.Equal(updatedKubeConfig.AuthInfos["remoteuser1"], "CHANGED TOKEN")
