...
func setupAppListEndpoint(k8s kubernetes.ClientInterface, config config.Config) (*httptest.Server, *prometheustest.PromClientMock) {
	prom := new(prometheustest.PromClientMock)

	// TODO: Do something about error
	business.SetupBusinessLayer(k8s, config)
	// business.SetupBusinessLayer()
...
func TestAppsEndpoint(t *testing.T) {
	...
	for _, obj := range business.FakeDeployments(cfg.IstioLabels.AppLabelName, cfg.IstioLabels.VersionLabelName) {
		o := obj
		kubeObjects = append(kubeObjects, &o)
	}
	k8s := kubetest.NewFakeK8sClient(kubeObjects...)
	k8s.OpenShift = true
	ts, _ := setupAppListEndpoint(k8s, *cfg)
	defer ts.Close()
