...
func TestStrategyTokenAuthentication(t *testing.T) {
	...
	authController := authentication.NewTokenAuthController(authentication.CookieSessionPersistor{}, mockClientFactory, cache, *cfg)
...
func TestLogoutWhenNoSession(t *testing.T) {
	...
	authController := authentication.NewTokenAuthController(authentication.CookieSessionPersistor{}, mockClientFactory, cache, *conf)
...
func TestStrategyHeaderOidcAuthentication(t *testing.T) {
	...
	k8s := kubetest.NewFakeK8sClient(&osproject_v1.Project{ObjectMeta: meta_v1.ObjectMeta{Name: "tutorial"}})
	k8s.OpenShift = true
	authController := authentication.NewHeaderAuthController(authentication.CookieSessionPersistor{}, k8s)
