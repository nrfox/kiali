...
class OAuthLogin implements LoginStrategy<unknown> {
  ...
  public async perform(request: any): Promise<LoginResult> {
    // get the data from the url that was passed by the OAuth login.
    // TODO: Here is where authenticate is being called?
    // Pass cluster here?
    const session = (await API.checkOpenshiftAuth(window.location.hash.substring(1), request.data)).data;
