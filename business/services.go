	...
	if criteria.Health {
		for i, sv := range services.Services {
			healthCriteria := HealthCriteria{
				Duration: criteria.RateInterval,
				QueryTime: criteria.QueryTime,
			}
			services.Services[i].Health, err = in.businessLayer.Health.GetServiceHealth(ctx, sv.ParseToService(), healthCriteria)
			if err != nil {
...
func (in *SvcService) GetServiceDetails(ctx context.Context, namespace, service, interval string, queryTime time.Time) (*models.ServiceDetails, error) {
	...
	go func(ctx context.Context) {
		...
		hth, err2 = in.businessLayer.Health.GetServiceHealth(ctx, &svc, criteria)
		if err2 != nil {
			errChan <- err2
		}
	}(ctx)
