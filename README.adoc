...
Kiali itself is instrumented with opentelemetry tracing to help provide insights and surface performance issues for the kiali server. To enable, set the `server.tracing_enabled` and `server.collector_url` configuration options.
